...
#import "JOHistoryViewController.h"

@interface JOHistoryViewController ()

@end

@implementation JOHistoryViewController

NSArray *hisres;
int myid, reload;
UISegmentedControl *hsc;
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = @"譲渡履歴";
    ...
    hsc = [[UISegmentedControl alloc] initWithItems:[NSArray arrayWithObjects:@"他人の出品", @"自分の出品", nil]];
    hsc.selectedSegmentIndex=1;
    hsc.segmentedControlStyle = UISegmentedControlStyleBar;
    hsc.tintColor = [UIColor colorWithRed:0.9 green:0.8 blue:0.0 alpha:1.0];
    self.navigationItem.titleView = hsc;
    [hsc addTarget:self action:@selector(segment:) forControlEvents:UIControlEventValueChanged];

    history2 = [self.storyboard instantiateViewControllerWithIdentifier:@"history2"];

    reload = 0;
}

- (void)viewDidAppear:(BOOL)animated{
    if(reload == 0 || hisres == nil || [hisres isEqual:[NSNull null]]){
        [self gethistory];
        reload = 1;
    }
}
...
- (void)gethistory{
    ...
    NSURL *url = [NSURL URLWithString:URL_USER_HISTORY];
    //通信
...
- (void)draw{
    if([hisres count] != 0){
        for(int i=0 ; i<[hisres count] ; i++){
            NSString *negoinfo = [hisres objectAtIndex:i];
            ...
            int uid = [[negoinfo valueForKeyPath:@"buyer_user_id"] intValue];
...
- (void)btn:(UIButton *)sender{
    NSString *negoinfo = [hisres objectAtIndex:sender.tag];
    NSString *photo = [negoinfo valueForKeyPath:@"photo1"];
...
- (void)segment:(id)sender{
    [self.navigationController pushViewController:history2 animated:NO];
}
